...
# Some functions we outsourced to make the server.R more readable
source('setup.R')
...
shinyServer(function(input, output, session) {
  ...
  observe({
    ...
    map <- leafletProxy("wdimap", session) %>%
      ...
      addPolygons(data = countries2(), 
                ...
                popup = country_popup, 
                layerId = ~iso2c) %>%
      leaflet::addLegend(colors = c(RColorBrewer::brewer.pal(classes, input$colors), "#808080"),
  ...
  output$dchart <- renderDygraph({ 
    ind <- input$indicator
    # Getting the Data from the World Bank API
    df <- WDI(country = ctry(), 
              indicator = ind, 
              start = 1980, 
              end = 2013, 
